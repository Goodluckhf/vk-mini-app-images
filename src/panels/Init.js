...
export default function Init({ id, go, fetchData }) {
    ...
    useEffect(() => {
        const startApp = async () => {
            ...
            let v = (await bridge.send('VKWebAppStorageGet', {
                keys: [ 'v' ]
            })).keys[0].value
    ...
    if (loading) return <Panel id={id} style={{ minHeight: '100vh'}}><ScreenSpinner /></Panel>
    ...
    return 	<Panel id={id} style={{ minHeight: '100vh'}}>
        <div className='InitMenu'>
            ...
            <Footer>Нажимая «Запустить», Вы соглашаетесь с 
                <Link href='https://dev.vk.com/ru/user-agreement' target='_blank'> пользовательским соглашением</Link> и 
                <Link href='https://dev.vk.com/ru/privacy-policy' target='_blank'> политикой конфиденциальности</Link> 
            </Footer>
        </div>
	</Panel>
}