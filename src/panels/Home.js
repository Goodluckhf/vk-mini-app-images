...
const Home = ({ id, go, user, folders, setActivePhoto, setAva }) => {
	...
	const childEllements = activeFolder.photos.map((photo, index) => {
		if (index < offset + MAX_COUNT) {
			...
			return <div key={photo.id} onClick={() => {
				fetch(user.photo_max_orig).then(res => res.blob()).then(blob => {
					setAva(blob)
	...
	return <Panel id={id} style={{ minHeight: '100vh'}}>
...

		{
			...
			<div style={{ padding: '10px'}} hidden={loading}>
				<Button className="DefaultButton" onClick={getPhotos} stretched size="l">
					Загрузить еще
				</Button>
			</div>
			: null
		...
		<div className="Tabbar">
			...
			<HorizontalScroll>
				<div style={{ display: 'flex', width: 'max-content', padding: '10px 5px', paddingBottom: '30px' }}>
				{
					folders.map(folder => {
						return <Button 
							size="l"
							key={folder.path}
							className={folder.name === activeFolder.name ? 'DefaultButton' : null}
							onClick={() => {
								loadingTimeout()
								setOffset(0)
